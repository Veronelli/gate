...
- **Catálogo de productos**: búsqueda contra una API de catálogo externa configurable por la variable de entorno `CATALOG_API_URL`, usada **solo como catálogo de referencia**. Cada producto muestra imagen, nombre, marca y **precio sugerido basado en el catálogo** (con la aclaración visible de que es un precio sugerido de referencia, no el precio donde el usuario compra). Productos únicos por `id`, asociados a un place.
...
- `catalog-search`: Integración con una API de catálogo externa exclusivamente para búsqueda de productos y precio sugerido de referencia (sin checkout ni compra en el comercio del catálogo).
...
- **APIs externas**: llamadas salientes a la API de catálogo configurada en `CATALOG_API_URL`, usada solo como catálogo de referencia; la API es pública y no requiere clave.
