...
- **Catálogo de productos**: búsqueda contra la API de intelligent-search de Día (`https://diaonline.supermercadosdia.com.ar/api/intelligent-search/v1/product-search?query={producto}`, sin clave) usada **solo como catálogo de referencia**. Cada producto muestra imagen, nombre, marca y **precio sugerido basado en precios de Día** (con la aclaración visible de que es un precio sugerido de referencia, no el precio donde el usuario compra). Productos únicos por `id`, asociados a un place.
...
- `dia-search`: Integración con la API de intelligent-search de Día exclusivamente para búsqueda de productos y precio sugerido de referencia (sin checkout ni compra en Día).
...
- **APIs externas**: llamadas salientes a `diaonline.supermercadosdia.com.ar` (intelligent-search de Día) usadas solo como catálogo de referencia; la API es pública y no requiere clave.
